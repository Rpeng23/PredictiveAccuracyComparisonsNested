#' Forecasting h-steps ahead using Recursive Least Squares
#'
#' Consider the following LS-fitted Model (with intercept):
#' y_{t+h} = beta_0 + x'_t * beta + u_{t+h}
...
recursive_hstep_fast=function(y,x,pi0,h)
{
  ...
  iXmatk0 = Matrix::solve(t(x[1:(k0-h),])%*%x[1:(k0-h),]) #M_k0
...
recursive_hstep_slow=function(y,x,pi0,h){
  ...
  for (s in k0:n)
  {
    bhat[,(s-k0+1)]=Matrix::solve(t(x[1:(s-h),])%*%x[1:(s-h),])%*%t(x[1:(s-h),])%*%y[(h+1):s]
  }
